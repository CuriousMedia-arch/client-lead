...
function parseContactSheet(text) {
  ...
  for (const row of rows.slice(1)) {
    ...
    contacts.push({
      ...
      email: get(row, "email"),
      phone,
      // Named for the DATABASE columns, not for what reads well here. These
      // used to be phone_alt / phone_alt_type while the importer read
      // p.phone2 / p.phone2_type, so every second mobile number in every sheet
      // was quietly thrown away — the value was simply undefined by the time
      // it reached the INSERT, and undefined becomes NULL without complaint.
      phone2: phoneAlt,
      phone_type: phone ? "mobile" : null,
      phone2_type: phoneAlt ? "mobile" : null,
  ...
  /*
   * Merge people who appear more than once in the sheet.
   *
   * Exports repeat a person across rows, each row carrying a different subset
   * of their details. Two reasons this has to happen here:
   *
   *  - Correctness: the second row's email and the first row's phone both
   *    belong on one record, and processing them separately meant whichever
   *    came last decided the outcome.
   *  - Necessity: the importer now writes contacts in multi-row batches, and
   *    Postgres rejects an upsert that names the same conflict key twice
   *    ("ON CONFLICT DO UPDATE command cannot affect row a second time").
   */
  const merged = new Map();
  for (const c of contacts) {
    const key = `${c.company.toLowerCase()}|${c.name.toLowerCase()}`;
    ...
    for (const [field, value] of Object.entries(c)) {
      if (field === "is_primary") seen.is_primary = seen.is_primary || value;
      else if (!seen[field] && value) seen[field] = value;
    }
  }
  ...
  return {
    ...
    contacts: deduped,
    // How many rows collapsed into someone already counted. Useful when the
    // row count and the contact count don't match and nobody can see why.
    mergedRows: contacts.length - deduped.length,
    skipped,
    headers: rows[0],
    matched,
    unmatched,
