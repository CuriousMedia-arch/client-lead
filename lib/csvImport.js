...
const COLUMNS = {
  ...
  domain:     ["companydomain", "domain", "website", "companywebsite"],
  city:       ["city", "location"],
  industry:   ["companymainindustry", "companyoldindustry", "industry", "companysubindustry"],
};
...
function parseContactSheet(text) {
  ...
  for (const row of rows.slice(1)) {
    ...
    if (!companies.has(key)) {
      companies.set(key, { name: company, keywords: keywordsFor(company) });
    }
    ...
    contacts.push({
      ...
      phone: get(row, "phone") || get(row, "altPhone"),
...
module.exports = { parseCsv, parseContactSheet, keywordsFor };
