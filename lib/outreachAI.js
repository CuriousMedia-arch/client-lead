...
/**
 * Wording an admin maintains, or null if they haven't filled it in.
 *
 * Never throws — a missing templates table (an older deployment that hasn't run
 * the migration) has to degrade to "no template", not break every pitch.
 */
async function template(key) {
  try {
    const row = await db.one("SELECT body FROM content_templates WHERE key = $1", [key]);
...
/** {{placeholder}} substitution. Unknown keys are left visible, not blanked. */
function fill(text, ctx) {
  const values = {
    company: ctx.company || "your team",
    contact: ctx.contact_name ? ctx.contact_name.split(" ")[0] : "there",
    contact_full: ctx.contact_name || "",
    role: ctx.contact_role || "",
    service: ctx.service || "",
    package: ctx.plan_name || "",
    price: ctx.price ? `₹${Number(ctx.price).toLocaleString("en-IN")}` : "",
    signal: ctx.signal_title || "",
    sender: ctx.sender || "Curious Media",
  };
  return String(text).replace(/\{\{\s*(\w+)\s*\}\}/g, (whole, k) =>
    values[k] != null && values[k] !== "" ? values[k] : whole
  );
}
...
async function recommendService(ctx) {
  // House rules, maintained by an admin in Settings. This is where the human
  // knowledge about which service suits which brand lives — the model on its
  // own only knows what services exist, not which ones Curious actually wants
  // to lead with for a given kind of client.
  const guidance = await template("service_guidance");

  const ai = await ask(
    `${CREDENTIALS}
${guidance ? `\nHouse rules from the sales team — follow these over your own judgement:\n${guidance}\n` : ""}
...
Return ONLY this JSON:
{
 ...
 "why":"<one sentence, max 25 words, saying which fact about this company you led with and why that angle>"
}`
  );
  ...
  return {
    ...
    why: ctx.signal_title
      ? "Opened on their recent news, then straight to what we'd run for them."
      : "No recent news on file, so this opens on what they're building rather than a specific trigger.",
    source: "rules",
  };
}
...
async function followupSuggestion(step, ctx = {}) {
  ...
  // If the sales team has written this one, use their words verbatim. A
  // human-written follow-up that the team has agreed on beats a generated one,
  // and generating over the top of it would quietly ignore the work.
  const written = await template(`followup_${step}`);
  if (written) {
    return {
      subject: `${ctx.company || "Curious Media"} × Curious Media`,
      body: fill(written, ctx),
      kind: plan.kind,
      source: "template",
    };
  }
