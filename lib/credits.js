 ...
 *
 * `spent` is what THIS claim actually cost, and every figure is worked out
 * against it — 15 for a Fresh Leads claim, 5 for a Newspaper pick-up. So the
 * proportions are identical on both tracks (a conversion always triples what
 * you put in) while a cheap claim can never pay out more than an expensive
 * one. Working the ladder off a fixed figure instead would make losing a
 * 5-credit lead after two replies profitable, and a rule that pays people to
 * lose is a rule they will eventually notice.
 *
 * The penalty is the exception: it is a flat number, not a proportion,
 * because "don't claim what you aren't going to work" is the same instruction
 * whatever the lead cost.
 */
function settlementFor(outcome, replyRounds, rules, spent) {
  // What was actually paid for this claim. Falls back to the Fresh price for
  // any caller that has not got a claim row to hand.
  const cost = Number.isFinite(Number(spent))
    ? Number(spent)
