...
/** Claim a lead, starting its clock. */
...
/** Undo a close, restarting the clock from what's left of the original window. */
...
/**
 * Release everything past its deadline.
 *
...
async function sweepExpired() {
  const expired = await db.all(
    `SELECT id, claim_source FROM leads
  ...
  const fresh = expired.filter((l) => l.claim_source === "fresh").map((l) => l.id);
  const rest = expired.filter((l) => l.claim_source !== "fresh").map((l) => l.id);

  if (fresh.length) {
    // released_at is what the Newspaper groups by, so it's stamped here and
    // nowhere else — updated_at moves whenever anything on the row changes.
    await db.run(
      `UPDATE leads
          SET owner_id = NULL, claimed_at = NULL, claim_source = NULL,
              deadline_at = NULL, pool = 'newspaper', status = 'new',
...
function countdown(lead) {
  if (!lead || !lead.deadline_at || lead.closed_at) return null;

  const msLeft = new Date(lead.deadline_at).getTime() - Date.now();
