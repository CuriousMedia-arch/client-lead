 ...
 */
const db = require("../db");
const freshCredits = require("./freshCredits");
...
/**
 * Give back everything a Fresh claim took.
 *
 * The contacts came with the company, so they leave with it — otherwise
 * releasing a lead you can't work leaves you holding its five people, and
 * nobody else can touch them. Contacts claimed on their own from All Leads
 * are untouched.
 */
async function releaseCascade(leadId, userId, note) {
  const company = await db.one(
    "SELECT c.name FROM leads l JOIN companies c ON c.id = l.company_id WHERE l.id = $1",
    [leadId]
  );
  if (!company) return { released: [] };
  ...
  const released = await db.all(
    `UPDATE company_contacts
        SET owner_id = NULL, claimed_at = NULL, deadline_at = NULL,
            ...
            release_note = $3,
            taken_from = NULL, taken_from_status = NULL
      WHERE lower(company) = lower($1)
        AND owner_id = $2
        AND claim_source = 'fresh'
        AND deleted_at IS NULL
      RETURNING id, name`,
    [company.name, userId, note || "Went back when the Fresh Lead was released"]
  );

  if (released.length) {
    await db.run(
      `UPDATE contact_claims SET released_at = now(), release_note = $3
        WHERE contact_id = ANY($1) AND user_id = $2 AND released_at IS NULL`,
      [released.map((r) => r.id), userId, note || null]
    );
  }

  return { company: company.name, released };
}
...
async function cascadeFreshClaim(leadId, userId) {
  ...
  // Swept contacts must expire WITH the Fresh claim that pulled them in, not
  // on the All Leads window. They were using windowFor("all") — thirty days —
  // so a contact borrowed for a ten-day news window stayed locked up for three
  // weeks after that window shut, invisible to everyone else.
  const lead = await db.one(
    `SELECT fresh_deadline_at FROM leads WHERE id = $1`,
    [leadId]
  );
  // lifecycle.claim() runs before this and sets the deadline, but a null here
  // would silently write an open-ended claim — so fall back to the Fresh
  // window rather than trusting call order forever.
  const until =
    (lead && lead.fresh_deadline_at) ||
    new Date(Date.now() + windowFor("fresh") * 86400000);
  ...
  const takenOver = await db.all(
    `SELECT cc.id, cc.name, cc.owner_id, u.display_name AS previous_owner
      ...
      WHERE lower(cc.company) = lower($1)
        ...
        AND cc.owner_id <> $2
        -- Only people whose All Leads owner has not actually messaged them
        -- yet. 'new' and 'working' mean claimed-but-nothing-sent; anything
        -- from 'contacted' onward is a live conversation and belongs to
        -- whoever started it, Fresh claim or not.
        AND cc.status IN ('new','working')
        AND cc.id NOT IN (
          SELECT contact_id FROM opportunities
           WHERE contact_id IS NOT NULL AND last_contacted_at IS NOT NULL
        )`,
    [company.name, userId]
  );
  ...
  const claimed = await db.all(
    `UPDATE company_contacts
      ...
      WHERE lower(company) = lower($1)
        ...
        AND (
              owner_id IS NULL
              -- Same rule as the read above: take over only what nobody has
              -- messaged yet.
              OR (
                   owner_id <> $2
      ...
      RETURNING id, name`,
    [company.name, userId, until]
  );

  // A swept contact is a claim like any other: it goes in the history, so the
  // count under the Claim button stays truthful, and re-claiming the same
  // person later doesn't count twice.
  if (claimed.length) {
    const ids = claimed.map((c) => c.id);
    await db.run(
      `INSERT INTO contact_claims (contact_id, user_id, source)
       SELECT unnest($1::bigint[]), $2, 'fresh'
...
async function sweepExpired() {
  ...
  if (expiredFresh.length) {
    // Settle first, for the same reason freshClock does: the UPDATE below is
    // about to remove the owner the settlement is paid to. This sweep writes
    // the columns directly rather than going through releaseFresh, so it
    // needs its own call.
    for (const l of expiredFresh) {
      await freshCredits.settle(l.id, { note: "Claim expired" }).catch(() => {});
    }
