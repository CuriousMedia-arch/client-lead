...
/**
 * A safe fingerprint of the key actually loaded — enough to tell two keys
 * apart when one works in a terminal and the other is in .env, without ever
...
async function healthCheck() {
  ...
  try {
    ...
    return { ok: true, model: MODEL, key, sample: text.slice(0, 80) };
  } catch (err) {
    return { ok: false, reason: err.message, model: MODEL, key };
  }
}

module.exports = { generate, healthCheck, configured, keyFingerprint, MODEL };
