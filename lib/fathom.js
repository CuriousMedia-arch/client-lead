 ...
 */
function verifySignature(rawBody, signature, secretOverride) {
  const secret = secretOverride || process.env.FATHOM_WEBHOOK_SECRET;
  if (!secret) return { ok: false, reason: "No Fathom webhook secret to check against." };
...
async function call(path, apiKey) {
  const res = await fetch(`${API}${path}`, {
    headers: { "X-Api-Key": apiKey || process.env.FATHOM_API_KEY, Accept: "application/json" },
  });
...
async function transcriptFor(recordingId, apiKey) {
  const data = await call(`/recordings/${recordingId}/transcript`, apiKey);
...
/**
 * Whose Fathom account did this webhook come from?
 *
 * Every connected account's secret is tried against the signature. Only the
 * account that actually produced the recording can match, so this both
 * authenticates the request and identifies the sender in one step.
 *
 * A loop over ~4 accounts, on a webhook that fires a few times a day. The
 * alternative — a shared secret — meant only one person's notes ever arrived
 * and everyone else's were rejected with no visible cause.
 */
async function accountForWebhook(rawBody, signature) {
  const accounts = await db.all(
    "SELECT user_id, api_key, webhook_secret, label FROM fathom_accounts"
  );

  for (const a of accounts) {
    let secret;
    try {
      secret = decrypt(a.webhook_secret);
    } catch {
      continue;   // key rotated or corrupt — skip rather than fail the batch
    }
    if (verifySignature(rawBody, signature, secret).ok) {
      return {
        userId: a.user_id,
        label: a.label,
        apiKey: a.api_key ? decrypt(a.api_key) : null,
      };
    }
  }

  // Fall back to the single shared secret, so a deployment that was set up
  // before per-user accounts keeps working without anyone re-entering anything.
  if (process.env.FATHOM_WEBHOOK_SECRET && verifySignature(rawBody, signature).ok) {
    return { userId: null, label: "shared", apiKey: process.env.FATHOM_API_KEY || null };
  }

  return null;
}

async function saveAccount(userId, { apiKey, webhookSecret, label }) {
  await db.run(
    `INSERT INTO fathom_accounts (user_id, api_key, webhook_secret, label, connected_at, last_error)
     VALUES ($1, $2, $3, $4, now(), NULL)
     ON CONFLICT (user_id) DO UPDATE SET
       api_key = EXCLUDED.api_key, webhook_secret = EXCLUDED.webhook_secret,
       label = EXCLUDED.label, connected_at = now(), last_error = NULL`,
    [userId, apiKey ? encrypt(apiKey) : null, encrypt(webhookSecret), label || null]
  );
}

const forgetAccount = (userId) =>
  db.run("DELETE FROM fathom_accounts WHERE user_id = $1", [userId]);

const accountFor = (userId) =>
  db.one(
    "SELECT user_id, label, connected_at, last_seen_at, last_error FROM fathom_accounts WHERE user_id = $1",
    [userId]
  );
...
async function summaryFor(recordingId, apiKey) {
  const data = await call(`/recordings/${recordingId}/summary`, apiKey);
