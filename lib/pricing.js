...
function quoteHealth({ price, vendorCost, internalCost, listPrice }, rules) {
  ...
  if (revenue > 0 && marginPct < floor) {
    reasons.push(
      `Margin is ${marginPct.toFixed(1)}%, below the approved floor of ${floor}%.`
    );
  }
  if (discountPct > maxDiscount) {
    reasons.push(
      `Discount is ${discountPct.toFixed(1)}% off the ${list.toLocaleString("en-IN")} list price, past the ${maxDiscount}% cap.`
    );
  }
