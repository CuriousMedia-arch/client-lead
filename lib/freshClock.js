 ...
 * stage, claiming a contact at that company. It resets the idle clock and,
 * the first time, promotes the claim off its 24-hour probation.
 */
async function touch(leadId) {
  // The first piece of activity ends probation: the 24-hour deadline becomes
  // the full window, measured from when it was claimed. Later activity only
  // resets the idle clock — it can't keep extending the close-by date.
  return db.run(
    `UPDATE leads
        SET fresh_last_activity_at = now(),
            fresh_warned_at = NULL,
            fresh_deadline_at = CASE
              WHEN fresh_from_newspaper THEN NULL
              WHEN fresh_deadline_at IS NULL THEN NULL
              WHEN fresh_deadline_at < fresh_claimed_at + interval '${RULES.closeByDays} days'
                THEN fresh_claimed_at + interval '${RULES.closeByDays} days'
              ELSE fresh_deadline_at
            END
      WHERE id = $1`,
    [leadId]
  );
...
async function releaseFresh(leadId, { note = null, toNewspaper = null } = {}) {
  ...
  // Settle the credits before the owner is wiped off the row — settlement
  // needs to know who held it and since when, and that is exactly what the
  // UPDATE below is about to erase. Every automatic exit (12h idle, 24h
  // release, 7 days no reply, 15 days unclosed) comes through here, so this
  // one call covers all of them. It is idempotent, so a path that already
  // settled explicitly — a manual release with a note — is not settled twice.
  const settlement = await freshCredits.settle(leadId, { note }).catch(() => null);
  ...
  if (settlement) row.settlement = settlement;
  return row;
}
