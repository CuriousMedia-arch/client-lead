...
/**
 * @param {string} mode  'company' (watchlist sweep) | 'new' (discovery sweep)
 *                       Passed through as a workflow input so the two Sync
 *                       buttons drive the two cadences independently.
 */
async function triggerRemoteRun(mode = "company") {
  if (!remoteRunConfigured()) throw new Error("Remote runs are not configured.");

  try {
    await axios.post(
      `https://api.github.com/repos/${REPO}/actions/workflows/${WORKFLOW}/dispatches`,
      { ref: BRANCH, inputs: { mode: mode === "new" ? "new" : "company" } },
      {
        timeout: 10_000,
        headers: {
